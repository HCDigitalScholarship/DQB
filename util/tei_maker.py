...
cnx3 = mysql.connector.connect(user='root', password='',
                               host='127.0.0.1',
                               database='ocr')
...
init_query = ('SELECT doc_name FROM ocr.words group by doc_name')
...
for doc in doc_names_list:
    ...
    lb_query = ("select a.doc_id,a.doc_name,a.word,a.y3,a.y3 - COALESCE(b.y3,0) as DIFF_to_Prev from ocr.words a LEFT OUTER JOIN ocr.words b ON (a.doc_id = b.doc_id + 1) WHERE a.doc_name = '%s' order by doc_id" % doc_name)
